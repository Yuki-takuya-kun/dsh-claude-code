...
- 上下文长度监控（lib/trace.mjs + lib/driver.mjs）：把 Claude SDK 的用量补进 DSH 会话日志——`message_start`/`message_delta` 合成 `TokenUsage` 挂在每步 `assistant/message` 上，`result.modelUsage` 的 `contextWindow` 映射成 `request/context`（经 sink 的 `requestContext()` 去重），token-meter 据此折叠出 `contextPressure`，Web 界面即可像 DSH 原生一样显示上下文占用比例。
