...
import { isWithinWorkspace } from "./permission.mjs";
...
/**
 * 生成 canUseTool 回调（或 null）。
 * @param ctx 主 cordis context（取 approval / userQuestions 服务）
 * @param {cwd, strategy, agent} cwd 会话工作区根目录；strategy 来自 permissionStrategy()；
 *   agent 为当前 ClaudeCodeAgent，供 approval.request / userQuestions.ask 定位所属会话
 * @returns {Promise<PermissionResult>} 或 null（strategy === "bypass" 时，由 driver 设 bypassPermissions）
 */
export function makeCanUseTool(ctx, { cwd, strategy, agent } = {}) {
  // danger-full-access：driver 设 permissionMode=bypassPermissions 完全放行，这里不设 canUseTool
