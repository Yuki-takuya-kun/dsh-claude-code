...
/**
 * request/header 的 system 占位文本：Claude Code 的系统提示词由 claude CLI 内部生成，
 * SDK 不暴露原文，只能写一句说明占位，让 token-meter 的 contextBreakdown 投影
 * systemTokens 至少落一个非零且诚实的读数（真实大小反映在占用条的总输入里）。
 */
const SYSTEM_STUB = "Claude Code 内置系统提示词（含工具说明与环境信息；原文由 claude CLI 内部生成，SDK 不暴露）";
...
const TOOL_NAME_MAP = {
  ...
  // 子代理委派工具（Agent 为新名、Task 为旧名）映射到 DSH 原生 subagent 卡片，
  // 让主轨迹里的「发起委派」显示成与 DSH 原生一致的子代理工具卡片。
  Agent: "subagent",
  Task: "subagent",
};

/** 会 spawn 子代理的 Claude Code 工具名集合（权威检测信号，见 lib/subagents.mjs）。 */
...
/**
 * 判断一条 Claude Agent SDK 消息是否属于「子代理」，需要从主轨迹里剔除：
 * - `system` 的 `task_started` / `task_progress` / `task_notification` / `task_updated`
 *   是子代理（Task/Agent 工具）的生命周期事件；
 * - 其余消息（assistant / user / stream_event / tool_progress）带非空
 *   `parent_tool_use_id` 即子代理的转发内容。
 * 仅在「关闭子代理追踪」（traceSubagents: false）时用于丢弃，避免污染主轨迹；
 * 开启追踪时的归位由 SubagentSessionManager 完成。
 */
export function isSubagentMessage(message) {
...
export class ClaudeRunTracer {
  ...
  #configContextWindow;
  /** 最近一次已写入的 request/header（system/tools/config），用于变化去重；未写为 null。 */
  #lastHeader = null;
  /** system/init 声明的内建工具名（Claude Code 名），映射成 ToolSchema 占位；null 表示未取到。 */
  #tools = null;
  #ended = false;
  ...
  #onInit(msg) {
    ...
    this.#emitRequestHeader();
  }

  /**
   * 把本轮请求的信封（system + tools）写成 `request/header`，token-meter 的
   * contextBreakdown 投影据此给出 systemTokens / toolsTokens（对话部分由 surface
   * 折叠给出）。只在首轮或 model/tools 变化时写，避免每轮重复。
   */
  #emitRequestHeader() {
    const header = {
      config: { provider: PROVIDER, model: this.#model },
      system: SYSTEM_STUB,
      ...(this.#tools && this.#tools.length > 0 ? { tools: this.#tools } : {}),
    };
    // 跨轮去重：优先比对会话里已折叠的 request/header（sink.requestHeader），
    // 没有时退回本轮实例内的 #lastHeader，避免每轮重复写 "initial"。
    const prev = this.#sink.requestHeader?.() ?? this.#lastHeader;
    if (prev
      && prev.system === header.system
      && prev.config.model === header.config.model
      && JSON.stringify(prev.tools ?? null) === JSON.stringify(header.tools ?? null)) {
      this.#lastHeader = header;
      return;
    }
    this.#sink.append("request/header", { header, reason: prev ? "change" : "initial" });
    this.#lastHeader = header;
  }
