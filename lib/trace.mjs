...
/**
 * 从 result 的 `modelUsage`（`Record<model, ModelUsage>`）取主模型（或唯一条目）的
 * `contextWindow`。只返回正整数——token-meter 的投影 schema 要求 `contextWindow` 为正，
 * 非正数会污染检查点校验。取不到时返回 undefined。
 */
export function contextWindowOf(modelUsage, model) {
  ...
  if (entries.length === 1) return positive(entries[0]?.contextWindow);
  return undefined;
}
...
export class ClaudeRunTracer {
  ...
  #model = "claude-code";
  /** 当前 step 的 token 用量（由 message_start 定输入/缓存、message_delta 定输出），无样本时为 null。 */
  #usage = null;
  #ended = false;
  ...
  #onStreamEvent(msg) {
    ...
    switch (event.type) {
      case "message_start": {
        ...
        const usage = usageFromClaude(event.message?.usage);
        // message_start 只携带输入侧（input + cache）；输出要等 message_delta 才定稿
        if (usage) this.#usage = { ...usage, outputTokens: 0 };
        break;
      }
      ...
      case "message_delta": {
        ...
        if (!usage) break;
        if (this.#usage) {
          // 输出 token 只在 message_delta 定稿；输入/缓存沿用 message_start 的读数（二者在流式期间不变）
          this.#usage.outputTokens = usage.outputTokens;
        } else {
          // 没有 message_start 的退化场景：直接用 delta 的完整 usage
          this.#usage = usage;
        }
        break;
      }
  ...
  #onAssistant(msg) {
    ...
    const data = { turn: this.#turn, step: this.#step, message: assistant };
    if (this.#usage) data.usage = this.#usage;
    this.#sink.append("assistant/message", data, opts);
    // 用量随本轮 assistant 消息入账；下一步的 sample 由下一个 message_start 重新建立
    this.#usage = null;
  ...
  /** 从 result 的 modelUsage 取主模型上下文窗口，变更时写入 `request/context`。 */
  #emitRequestContext(msg) {
    const contextWindow = contextWindowOf(msg?.modelUsage, this.#model);
