...
/** 把 session 包装成 tracer 需要的 sink（append 返回带 seq 的事件）。 */
function sessionSink(session) {
  return { append: (type, data, opts) => session.append(type, data, opts) };
}
...
export async function runClaudeCode({ ctx, config, session, agent, turn, prompt, signal, cwd, executable, env, resumeSessionId }) {
  const tracer = new ClaudeRunTracer(sessionSink(session), turn);
  ...
  // DSH 权限 → canUseTool 策略（sandbox/mode + approval/policy 已在会话创建时钉进事件流）
  const permission = readPermission(session);
  const strategy = permissionStrategy(permission);
  const canUseTool = makeCanUseTool(ctx, { cwd, strategy, agent });
  ...
  const options = {
    ...
    // danger-full-access：完全跳过 Claude 权限/沙箱（文件写入、Bash 等全放行）
    ...(strategy === "bypass"
      ? { permissionMode: "bypassPermissions", allowDangerouslySkipPermissions: true }
      : {}),
    // 其余策略桥接 DSH 审批 / 区内放行（bypass 时不设 canUseTool）
