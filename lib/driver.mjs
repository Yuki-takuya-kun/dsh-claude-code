...
import { ClaudeRunTracer, isSubagentMessage } from "./trace.mjs";
...
/** 把 session 包装成 tracer 需要的 sink（append 返回带 seq 的事件；requestContext / requestHeader 供去重）。 */
 ...
 * @param subagents 子代理会话管理器；为 null 表示未开启子代理追踪（子代理消息被丢弃）
 * @returns { output, stopReason, claudeSessionId }
 *   stopReason ∈ 'completed' | 'aborted' | 'error'
 */
export async function runClaudeCode({ ctx, config, session, agent, turn, prompt, signal, cwd, executable, env, resumeSessionId, subagents }) {
  ...
  const options = {
    ...
    // 子代理全文转发：仅当开启子代理追踪时才要求 SDK 转发子代理的 text/thinking 块。
    forwardSubagentText: subagents != null,
  ...
  try {
    ...
    for await (const message of q) {
      ...
      // 子代理消息：开启追踪时路由到子 session；未开启时丢弃，避免污染主轨迹。
      if (subagents != null) {
        if (subagents.handleMessage(message)) continue;
    ...
    tracer.finish();
    subagents?.finishRun({ aborted: controller.signal.aborted });
    ...
    tracer.finish();
    subagents?.finishRun({ aborted: controller.signal.aborted || signal.aborted });
