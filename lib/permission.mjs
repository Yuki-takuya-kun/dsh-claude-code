...
// 这两个事件在 DSH 会话创建时由 permission-presets 钉进初始值，运行中用户可在
// Web 界面 / permission 命令里切换 preset，此时会再追加一条同类型事件覆盖之——
// 所以 last-event-wins 的 fold（readPermission）必须每次现读，不能缓存回合起点快照。
